...
public class PatchRequest_payloadusingString_NONBDDStyle {
@Test
    public void patchRequest()
    {
       ...
       // Non BDD Style to extract token from the request to use it to the next request
        ...
        // patch request - Give the extracted token and id into Put request
        ...
        httprequest.body(payload_patch);
        response = httprequest.when().patch();
        validate = response.then().log().all();
        validate.statusCode(200);
        validate.body("firstname", Matchers.equalTo("James"));
